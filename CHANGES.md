 ...
 * test: don't use sandbox for 'src --version' (Ryan Graham)
 ...
 * Use strong-mesh-models for 'src ctl' (Ryan Graham)
 ...
 * bin: fix 'src build' linking (Ryan Graham)
 ...
 * Rename `src pmctl` to `src ctl` (Sam Roberts)
 ...
 * Add `src start` command (Sam Roberts)
 ...
 * Fix typo in src.txt (Jorrit Schippers)
 ...
 * src: rewrite usage message (Sam Roberts)
 ...
 * Add `src arc` command (Miroslav Bajtoš)
 ...
 * src: describe as "StrongLoop Controller" (Sam Roberts)
 ...
 * Fix formatting in src man page (Sam Roberts)
 ...
 * Add support for src loopback commands that delegate to yeoman (Raymond Feng)
 ...
 * Add a notice to `src example` command (Raymond Feng)
 ...
 * Add `src registry` as strong-registry/sl-registry (Miroslav Bajtoš)
 ...
 * example,lb: suggest `src strongops` before run (Sam Roberts)
 ...
 * version: add peer dependencies to `src --version` (Sam Roberts)

 * Use `src run` instead of `src run .` in doc output (Sam Roberts)
 ...
 * Change reference to `src help` to `src --help` (Sam Roberts)
 ...
 * src update command (Sam Roberts)
 ...
 * Add `src version` as alias for `src --version` (Sam Roberts)
 ...
 * Implement `src debug` as node-inspector/node-debug (Sam Roberts)

 * Implement `src clusterctl` with clusterctl (Sam Roberts)

 * Implement `src run` as strong-supervisor/sl-run (Sam Roberts)
 ...
 * Remove unused bin/src.cmd (Sam Roberts)

 * Remove slnode as an alias for src (Sam Roberts)
 ...
 * Commit src-create into src (Sam Roberts)
 ...
 * src version no longer should print suite version (Sam Roberts)

 * Install examples from data/, not node_modules/ (Sam Roberts)

 * Test process.env.SRC if defined (Sam Roberts)
 ...
 * Support --no-install for `src lb project` (Sam Roberts)
 ...
 * Added clusterctl.md to docs for new clusterctl sub-command in SRC 1.1. (Rand McKinney)
 ...
 * Fetching src-create from re branch (cgole)
 ...
 * Fix src lb model -i number parsing (SLA-614) (Ritchie Martori)
 ...
 * Add strong-cluster-control cli as an src command (Sam Roberts)

 * Fix src create with a nested path. (Sam Roberts)
 ...
 * Point master of src at master branch of src-create (Sam Roberts)
 ...
 * Improve `src lb project` UX. (Michael Schoonmaker)

 * Rename `src lb api` to `src lb project`. (Michael Schoonmaker)
 ...
 * Add the docs module to `src lb project` (Michael Schoonmaker)
 ...
 * Changed tag for src-create to trongloopsuite-1.0.0-3 (cgole)
 ...
 * Document the --no-install option to src create (Sam Roberts)
 ...
 * Add `src example` command for creating projects (Sam Roberts)
 ...
 * Remove unimplemented `src clean` command (Sam Roberts)
 ...
 * Use src-create as a module instead of pulling out a sub module (Ritchie Martori)

 * Refactor the create command (Ritchie Martori)

 * Move the CLI bits of src-create to the src loader api (Ritchie Martori)
 ...
 * Add node, npm, and src paths to env output (Sam Roberts)
 ...
 * Module not found is not an error for src commands (Sam Roberts)
 ...
 * Implement the 'src strongops' command (Edmond Meinfelder)

 * Order src command to first on doc page (Sam Roberts)
 ...
 * Rename slnode cli repositories to src (Sam Roberts)

 * Rename slnode cmd to src (Sam Roberts)
