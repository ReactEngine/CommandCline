...
function CommandLoader(obj) {
  ...
  obj = obj || {};
//通过obj.root 和 path.resolve(__dirname,'commands');来给this.root赋值  ,如果obj.root 不存在 就通过path.resolve(__dirname, 'commands');来拼接
  this.root = obj.root || path.resolve(__dirname, 'commands');
  this.strict = true;//是否按照严谨模式来执行  参照 ECMAscript 5 添加的use strict  严谨模式
  this.usage = obj.usage || 'help'; //用法
  this.fallback = obj.fallback || this.usage;//回退
  this.default = obj.default || this.usage;//如果没有定义默认的方式 就使用用法
  this.manuals = obj.manuals || null;//如果在rec.js里面没有传值过来 就让它变为null
}
util.inherits(CommandLoader, EventEmitter);//让CommandLoader继承events.EventEmitter让CommandLoader有事件触发与事件监听器功能   详见events.EventEmitter
CommandLoader.createLoader = CommandLoader;//CommandLoader创建一个property createLoader,把上面的CommandLoader赋值给它.
...
//
CommandLoader.prototype.parse = parse; //给CommandLoader创建一个命令行解析的功能方法.借助optimist库来实现命令行解析
...
//
CommandLoader.prototype.run = run; //创建run功能方法
function run(argv) {
  var self = this;
  var options = self.parse(argv || (argv = process.argv.slice(2)));
  var command = null;

  debug('rec.run:', 'argv', argv, 'options', options);

  if (argv[0] === '-h' || argv[0] === '--help') {
    return self.printUsage('rec');
    ...
    if (command) {
      ...
      return self.error(
        '"%s" is not an rec command. See `rec --help` for more information.',
        options._[0]);
    } else {
  ...
  process.env.REC_COMMAND = command.command;
...
//
CommandLoader.prototype.loadCommand = loadCommand; //返回一个命令或者返回一个null,
function loadCommand(name) {
  var self = this;
  var module = null;
  // Try to support rec loopback:model
  ...
  try {
    console.log("loader loadCommand !");
    console.log(command);
    module = require(command);
  } catch (e) {
    console.log('require %s failed with',command,e);
